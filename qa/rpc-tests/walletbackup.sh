...
function S {
  ...
  if [ x$TXID = x ] ; then
      echoerr "node${1}: error sending ${3} tgd"
...
function StartThree {
  $DIAMONDD $DIAMONDDARGS $B1ARGS &
  B1PID=$!
  $DIAMONDD $DIAMONDDARGS $B2ARGS &
  B2PID=$!
  $DIAMONDD $DIAMONDDARGS $B3ARGS &
  B3PID=$!
}
