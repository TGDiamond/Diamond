...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Diamond database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Diamond datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Diamond datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Diamond 0.7 datadir."
    ;;
  3)
    echo "Detected Diamond pre-0.8 datadir."
    ;;
  4)
    echo "Detected Diamond 0.8 datadir."
    ;;
esac
